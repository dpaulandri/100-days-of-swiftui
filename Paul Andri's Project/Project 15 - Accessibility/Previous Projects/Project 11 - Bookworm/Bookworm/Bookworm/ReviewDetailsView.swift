...
struct ReviewDetailsView: View {
    ...
    var body: some View {
        ...
        // VSTACK - SHOWS 'StarRatingView' & REVIEW DATA VSTACK VIEW
        // CONTAINS PART OF DAY 56 - CHALLENGE #3 CODE
        VStack {
            ...
            // REVIEW DATE VSTACK VIEW
            VStack {
                Text("Review Date")
                    .font(.headline)
                Text(review.date?.formatted(date: .long, time: .shortened) ?? "N/A")
                    .font(.subheadline)
            }
            .padding(.bottom, 5)
        }
