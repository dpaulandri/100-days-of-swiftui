//
//  FilteredChallengeView.swift
//  CoreDataProject
//
//  Created by Paul Andri on 08/14/2022.
//
// CONTAINS SOLUTION CODE FOR DAY 59: CHALLENGE #1 & #3
...
struct FilteredChallengeView<T: NSManagedObject, Content: View>: View {
    ...
    // PART OF DAY 59 - CHALLENGE #1:
    // MAKE CUSTOM INITIALIZER ACCEPT A 'String' PARAMETER TO CONTROL WHICH 'NSPredicate' TO APPLY
    /*
    // CUSTOM INITIALIZER FOR 'fetchRequest'' PROPERTY
    // ACCEPT TWO 'String' VALUES AS ITS 'NSPredicate' FILTER SYNTAX ARGUMENTS,
    
    // PART OF DAY 59 - CHALLENGE #1:
    // ACCEPT A 'String' PARAMETER TO CONTROL WHICH 'NSPredicate' TO APPLY
    ...
    init(filterKey: String, filterValue: String, predicate: String, @ViewBuilder content: @escaping (T) -> Content) {
        ...
        _fetchRequest = FetchRequest<T>(sortDescriptors: descriptor, predicate: NSPredicate(format: "%K \(predicate) %@", filterKey, filterValue))
         //SYNTAX EXPLANATION:
        /*
    ...
    */
    ...
    // MAKE CUSTOM INITIALIZER ACCEPT A "GENERIC" ARRAY OF 'SortDescriptor' OBJECT FOR 'sortDescriptor' PARAMATER VALUE
    // CUSTOM INITIALIZER FOR 'fetchRequest'' PROPERTY
    // ACCEPT TWO 'String' VALUES AS ITS 'NSPredicate' FILTER SYNTAX ARGUMENTS,
    // AND AN "ESCAPING" CONTENT CLOSURE "VIEWBUILDER" THAT ACCEPT A "GENERIC" OBJECT & RETURNS A 'Content'
    // ACCEPT A 'String' PARAMETER TO CONTROL WHICH 'NSPredicate' TO APPLY (PART OF DAY 59 - CHALLENGE #1)
    ...
    // '@ViewBuilder' - LET THE CONTAINING 'View' SEND IN MULTIPLE 'View's IF NEEDED
    // '@escaping' - MARK THE CLOSURE WILL BE STORE AWAY & BE USED LATER
    init(filterKey: String, filterValue: String, predicate: String, descriptor: [SortDescriptor<T>], @ViewBuilder content: @escaping (T) -> Content) {
        // "GENERIC" 'NSPredicate' SORTING:
        // '_fetchRequest' PASSES A NEW 'FetchRequest' FILTERED OBJECT DIRECTLY FOR 'fetchRequest' TO USE
        // SORT THE FETCHREQUEST DATA USING THE PASSED-IN INITIALIZER 'filter' VALUE AGAINST 'keyName' W/ 'NSPredicate'
        _fetchRequest = FetchRequest<T>(sortDescriptors: descriptor, predicate: NSPredicate(format: "%K \(predicate) %@", filterKey, filterValue))
